...
@AndroidEntryPoint
class FindPasswordChangeFragment :
    UtilityBase.BaseFragment<FragmentFindPasswordChangeBinding>(R.layout.fragment_find_password_change) {
    ...
    suspend fun passwordChangeAuthoritySend() {
        if (TextUtils.isEmpty(binding.passwordEdit.text) || TextUtils.isEmpty(binding.checkEdit.text)) {
            binding.checkPasswordErrorText.errorAnimationShow(
                ...
                binding.checkEdit.text
            )
        ) {
            binding.passwordCheckErrorText.errorAnimationShow(
                requireContext(),
                "영어, 숫자, 특수문자만을 섞어 8~16자로 다시 입력해주세요."
            )
            ...
            viewModel.putPasswordChange(
                binding.passwordEdit.text.toString(),
                binding.checkEdit.text.toString()
            )
        }
    }
